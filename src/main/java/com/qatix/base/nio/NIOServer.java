...
/**
 * @see https://blog.csdn.net/u014730165/article/details/85089085
 */
public class NIOServer {
    ...
    private void listen() throws IOException {
        ...
        while (i++ < 10) {
            // 选择一组键，并且相应的通道已经打开
            /**
             * epoll底层维护一个链表，rdlist，基于事件驱动模式，
             * 当网卡有数据请求过来，会发起硬件中断，通知内核已经有来了。
             * 内核调用回调函数，将当前的事件添加到rdlist中，将当前可用的rdlist列表发送给用户态，
             * 用户去遍历rdlist中的事件，进行处理
             */
            selector.select();
            // 返回此选择器的已选择键集。
    ...
    private void handleKey(SelectionKey selectionKey) throws IOException {
        ...
        } else if (selectionKey.isWritable()) { //收到可写事件后开始写出
            //将缓冲区清空以备下次写入
            sendbuffer.clear();
            // 返回为之创建此键的通道。
            client = (SocketChannel) selectionKey.channel();
